...

{
    ...
    if (pa!=null)
    {
        ...
        for (Address xad : add)
        {
            ...
            if (xad.hasCity()) {paddr=paddr+" - "+xad.getCity();}
            if (xad.hasState()) {paddr=paddr+" , "+xad.getState();}
            if (xad.hasCountry()) {paddr=paddr+" , "+xad.getCountry();}
            if (xad.hasPostalCode())  {paddr=paddr+" ("+xad.getPostalCode()+")";}
...
for (Bundle.BundleEntryComponent entry: bu.getEntry())
{
    ...
    String cCode=oneI.getCode().getCodingFirstRep().getCode()+":"+oneI.getCode().getCodingFirstRep().getDisplay();
...
if (aux==""){ aux="Error:No_Conditions";}     
return aux;
}
...
public String L00_4_CreateUSCoreAllergyIntolerance(
  ...
  AllergyIntolerance a=new AllergyIntolerance();
    ...
    //Clinical Status
    cClinicalStatus.addCoding().setSystem("http://terminology.hl7.org/CodeSystem/allergyintolerance-clinical").setCode(ClinicalStatusCode);
    a.setClinicalStatus(cClinicalStatus);
    //Verification Status
    CodeableConcept cVerificationStatus = new CodeableConcept();
    cVerificationStatus.addCoding().setSystem("http://terminology.hl7.org/CodeSystem/allergyintolerance-verification").setCode(VerificationStatusCode);
    ...
    cMani.addCoding().setSystem("http://snomed.info/sct").setCode(ManifestationSnomedCode).setDisplay(ManifestationSnomedDisplay);
...
private String GetLabResults(IGenericClient client, String id) {
    ...
    for (Bundle.BundleEntryComponent entry: bu.getEntry())
    {
                ...
                if ( m_category.equals("laboratory") )
                {
                  ...
                  if (u_code!=null)
                   ...
                   if (m_code!="")
                   {
                        ...
                        {m_result=oneP.getValueCodeableConcept().getCodingFirstRep().getCode()+":"+oneP.getValueCodeableConcept().getCodingFirstRep().getDisplay();}
        ...
        if (aux==""){ aux="Error:IPS_No_Observations";}     
    }
