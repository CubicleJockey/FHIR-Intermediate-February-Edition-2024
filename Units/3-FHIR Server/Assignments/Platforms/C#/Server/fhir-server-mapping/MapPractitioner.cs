    ...
    public static class MapPractitioner
    {
        public static Practitioner GetFHIRPractitionerResource(fhir_server_entity_model.LegacyPerson person)
        {
            ...
            if (!string.IsNullOrWhiteSpace(person.PRSN_SECOND_NAME))
            {
                ...
                p.Name[0].Text = $"{(string.IsNullOrWhiteSpace(person.PRSN_FIRST_NAME) ? string.Empty : person.PRSN_FIRST_NAME)} {(string.IsNullOrWhiteSpace(person.PRSN_SECOND_NAME) ? string.Empty : person.PRSN_SECOND_NAME)} {(string.IsNullOrWhiteSpace(person.PRSN_LAST_NAME) ? string.Empty : person.PRSN_LAST_NAME)}";
            }
